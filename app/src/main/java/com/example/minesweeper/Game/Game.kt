...
import com.example.minesweeper.Random.DoublyIndexedItem
...
open class Game(
    val height: Int, val width: Int, val amountOfMines: Int,
    private val winnableGame: Boolean = false,
    private var currentInputMode: InputMode = InputMode.FLAGGING
) : gamescreen() {
    open val board: List<List<IndexedTile>> = List(height) { i ->
        List(width) { j ->
            IndexedTile(
                i,
                j,
    ...
    private fun winConditionDone(): Boolean {
        return ((board.flatten().count { !it.value.isRevealed } == amountOfMines) or board.flatten()
            .filter { it.value.isMine }.all { it.value.isFlagged })
    ...
    open fun plantMines(startingTile: IndexedTile) {
        ...
        possibleMines.choose(amountOfMines - 1).forEach(::plantMine)
        countMines()
    }
...
class UnMutatingGame(
    height: Int, width: Int, amountOfMines: Int,
    winnableGame: Boolean = false,
    currentInputMode: InputMode = InputMode.FLAGGING
) : Game(height, width, amountOfMines, winnableGame, currentInputMode) {
    override val board: MutableList<MutableList<IndexedTile>> = MutableList(height) { i ->
        MutableList(width) { j ->
            IndexedTile(
                i,
                j,
