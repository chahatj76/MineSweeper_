...
class GameViewModel(height: Int, width: Int, amountOfMines: Int) : ViewModel() {
    val game: Game = UnMutatingGame(
        height,
        width,
        amountOfMines
    ).apply { endCallback = { winnState.value = it } }
    ...
    val tiles: List<MutableLiveData<Tile.State>> =
        board.map { MutableLiveData<Tile.State>().apply { value = it.value.state } }
    val gameState = MutableLiveData<Game.EndState>()
    fun flagsLeft_() = game.amountOfMines - board.count { it.value.isFlagged }
