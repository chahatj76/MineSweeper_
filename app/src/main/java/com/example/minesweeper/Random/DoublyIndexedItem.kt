package com.example.minesweeper.Random

import utils.cartesianProduct

data class DoublyIndexedItem<T>(val i: Int, val j: Int, val value: T) {
    val offsets =
        arrayListOf(0, 1, -1).cartesianProduct(arrayListOf(0, 1, -1)).minus(setOf(Pair(0, 0)))
    ...
    fun <T> Collection<Collection<T>>.inRange(i: Int, j: Int) =
        i in 0..(size - 1) && j in 0 until first().size
}